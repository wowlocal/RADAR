...
def create_checkouts_dir():
	checkouts_dir = os.path.join(current_dir, "checkouts")
...
def change_dependencies_to_local_sources(project_list):
	for project in project_list:
		...
		gradle_dependencies_file = os.path.join(project.local_path, project.name + "-lib", "internal_dependencies.gradle")
		# clean file
		open(gradle_dependencies_file, 'w').close()
		with open(gradle_dependencies_file, 'w') as f:
			f.write("dependencies {\n")
			for dependency in dependencies:
				f.write(f"\timplementation project(':../../{dependency}')\n")
			f.write("}\n")
...
def update_dependencies_commit(project_list):
	for project in project_list:
		dependencies_file = os.path.join(project.local_path, "dependencies")
		if not os.path.exists(dependencies_file):
			continue
		...
		with open(gradle_dependencies_file, 'w') as f:
			f.write("dependencies {\n")
			for dependency in dependencies:
				f.write(f"\timplementation ':{dependency}'\n")
			f.write("}\n")
